...
def _cfg(url='', **kwargs):
    return {
        'url': url,
        'num_classes': 21, 'input_size': (3, 256, 256), 'pool_size': None,
...
default_cfgs = {
    ...
    'vit_large_patch32_384': _cfg(
        url='https://github.com/rwightman/pytorch-image-models/releases/download/v0.1-vitjx/jx_vit_large_p32_384'
            '-9b920ba8.pth',
        input_size=(3, 384, 384), crop_pct=1.0),
    ...
    'vit_tiny_patch16_224_in21k': _cfg(
        url='https://storage.googleapis.com/vit_models/augreg/Ti_16-i21k-300ep-lr_0.001-aug_none-wd_0.03-do_0.0-sd_0'
            '.0.npz',
        num_classes=21843),
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.head_dist is not None:
            ...
            x = self.head(x)  # torch.Size([15, 21])
        return x
...
@register_model
def vit_tiny_patch16_256(pretrained=False, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
    model = VisionTransformer(img_size=256, patch_size=16, in_chans=3, num_classes=21, embed_dim=768)
    return model
...
@register_model
def vit_base_patch16_256(pretrained=True, **kwargs):
    """ ViT-Tiny (Vit-Ti/16)
    """
    model = VisionTransformer(img_size=256, patch_size=16, in_chans=3, num_classes=21, embed_dim=768)
    # model_kwargs = dict(patch_size=16, embed_dim=768, depth=12, num_heads=12, **kwargs)
    # model = _create_vision_transformer('vit_base_patch16_256', pretrained=pretrained, **model_kwargs)
    return model
